...
/**
 * Attachment_Service_AttachmentSerwis1
 *
@author Andrzej Wilczyński <and.wilczynski@gmail.com>
 */
class Attachment_Service_AttachmentSerwis1 extends MF_Service_ServiceAbstract {
    ...
    public function init() {
        ...
        $this->attachmentTable = Doctrine_Core::getTable('Attachment_Model_Doctrine_AttachmentSerwis1');
        parent::init();
    } 
    ...
    public function getAttachmentForm(Attachment_Model_Doctrine_AttachmentSerwis1 $attachment = null) {
    ...
    public function saveAttachmentFromArray($values) {
        ...
        foreach($languages as $language) {
            if(is_array($values['translations'][$language]) && strlen($values['translations'][$language]['title'])) {
                ...
                $attachment->Translation[$language]->slug = MF_Text::createUniqueTableSlug('Attachment_Model_Doctrine_AttachmentSerwis1Translation', $values['translations'][$language]['title'], $attachment->getId());
...
//            if($this->createAttachmentFile($filename, $filename)) {
                ...
                $attachment->Translation[$languageAdmin]->slug = MF_Text::createUniqueTableSlug('Attachment_Model_Doctrine_AttachmentSerwis1Translation', $title, $attachment->getId());
