 ...
 */
class Product_Service_Dimension extends MF_Service_ServiceAbstract {
    
    protected $dimensionTable;
    protected $dimensionPriceTable;
    
    public function init() {
        $this->dimensionTable = Doctrine_Core::getTable('Product_Model_Doctrine_Dimension');
        $this->dimensionPriceTable = Doctrine_Core::getTable('Product_Model_Doctrine_DimensionPrice');
        parent::init();
    }
   
    public function getDimension($id, $field = 'id', $hydrationMode = Doctrine_Core::HYDRATE_RECORD) {
        return $this->dimensionTable->findOneBy($field, $id, $hydrationMode);
    }
