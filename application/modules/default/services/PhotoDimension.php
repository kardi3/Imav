...
class Default_Service_PhotoDimension extends MF_Service_ServiceAbstract {
    ...
    public function getDimensionArray($id, $hydrationMode = Doctrine_Core::HYDRATE_RECORD) {
        ...
        $dimension = $this->photoDimensions->findOneById($id, $hydrationMode);
        if(!empty($dimension->width)||!empty($dimension->height))
            $dimensionArray[$dimension->width."x".$dimension->height] = $id;
        else
            $dimensionArray['200x200'] = 'Default photo';
